...
from guidance.config import BaseOptions
from guidance.start_end_dataset import \
    StartEndDataset, start_end_collate, prepare_batch_inputs
from guidance.inference import eval_epoch, start_inference, setup_model
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        pos_neg = [ sample['sample_type'] for sample in batch[0]] #positive and negative samples
        timer_start = time.time()
        outputs = model(**model_inputs)
        loss_dict = criterion(outputs, targets,pos_neg)
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        save_interval = 5  # small interval
        if (epoch_i + 1) % save_interval == 0:  # additional copies
            ...
            torch.save(checkpoint, opt.ckpt_filepath.replace(".ckpt", f"_e{epoch_i+1:04d}.ckpt"))
...
def start_training():
    ...
    if opt.eval_path is not None:
        ...
        dataset_config["q_feat_path"] = opt.t_feat_path # for pretraining
        dataset_config["eval_window"] = opt.eval_window # set false for untrimmed eval
        dataset_config["load_labels"] = False  # comment or set true to calculate eval loss
...
if __name__ == '__main__':
    ...
    if not debug:
        ...
        #start_inference() No need inference for now 
